...
**Estado**: revisado y aprobado por el propietario (2026-08-21).
...
| Rol | Quién | Hace qué |
|---|---|---|
| Quien detecta | Cualquiera | Documenta hora, qué vio, y avisa de inmediato a Elián Pereira — `pereiraelian18@gmail.com` / 9992689400 (WhatsApp o llamada) |
| Contención | Dev con acceso a Railway/Cloudflare | Ejecuta §3 |
| Decisión de negocio | Propietario (Elián Pereira) | Autoriza rotar credenciales de pago (Openpay), notificar a clientes, o congelar el sitio |
| Comunicación | Elián Pereira redacta y aprueba cualquier aviso a clientes |

**Canal de aviso inmediato**: `pereiraelian18@gmail.com` / 9992689400, a
Elián Pereira, sin importar la hora — hasta que exista un canal de guardia
más amplio (equipo, terceros), este es el único punto de contacto.
...
3. Si hay evidencia de fraude activo (cargos no autorizados, patrón de
   tarjetas robadas): decisión del propietario (Elián Pereira, mismo
   contacto del §2), incluye posible congelamiento temporal de nuevos
   cobros. El soporte de Openpay es **(55) 97 55 35 59 /
   soporte@openpay.mx** (ver Memoria).
...
Política de notificación a clientes, confirmada por el propietario
(2026-08-21):

- **Umbral de severidad para notificar a clientes**: solo cuando el
  incidente involucra datos fiscales o de pago (RFC, razón social, datos
  de tarjeta/transacción vía Openpay, montos cobrados). Un incidente que
  no toque esos datos (ej. una vulnerabilidad de XSS ya contenida sin
  evidencia de explotación) no dispara aviso a clientes.
- **Quién redacta y aprueba el mensaje**: Elián Pereira, en todos los
  casos.
- **Canal interno de aviso inmediato**: `pereiraelian18@gmail.com` /
  9992689400, a Elián Pereira (ver §2) — cualquier hora.
- Pendiente sin resolver: si el incidente activa una obligación legal de
  notificar bajo la legislación aplicable a `legal/` (ARCO) más allá del
  umbral de negocio de arriba — evaluar caso por caso con el propietario
  cuando se presente, no hay regla fija documentada todavía.
...
Los tres puntos que dependían de una decisión del propietario (canal de
aviso inmediato, umbral de notificación a clientes, quién redacta y
aprueba ese aviso) quedaron confirmados el 2026-08-21 (ver §2 y §6). Lo
único que sigue abierto es si un incidente concreto activa además una
obligación legal de notificación bajo ARCO — eso se evalúa caso por caso,
no tiene una regla fija todavía.
