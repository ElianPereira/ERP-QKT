...
19, 20, 21, 23, 24, 25, 26, 27, 29, 30 y 31 ya están hechas (Fase 0 y Fase 1
completas; de la Fase 2, rate limiting, CSRF/validación del cotizador, el
ocultamiento de detalles de excepción, el gate de lint en CI y el análisis
estático de seguridad de ruff ya están — queda la verificación del proxy
de Railway, orden 22, que depende de Infra). Las dos verificaciones externas
resultaron
...
5. Orden 32 — `SEC-CI-001d` (`S`): añadir `gitleaks` al pipeline de CI, ahora que el gate de lint (órdenes 29-31) ya bloquea de verdad.
