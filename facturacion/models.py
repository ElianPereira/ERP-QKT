...
class SolicitudFactura(models.Model):
    ...
    def __str__(self):
        folio = f"SOL-{self.id:04d}" if self.id else "Nueva"
        return f"{folio} | {self.cliente.nombre} | ${self.monto:,.2f}"
    ...
    def get_datos_para_contador(self):
        ...
        lineas = [
            "═══════════════════════════════════",
            f"📋 SOLICITUD DE FACTURA SOL-{self.id:04d}",
            "═══════════════════════════════════",
            "",
            "👤 DATOS FISCALES:",
            f"   RFC: {self.rfc}",
